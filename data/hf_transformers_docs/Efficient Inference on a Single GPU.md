...
After installing the [`optimum`](https://github.com/huggingface/optimum) package, to use Better Transformer during inference, the relevant internal modules are replaced by calling [to\_bettertransformer()](/docs/transformers/v4.34.0/en/main_classes/model#transformers.PreTrainedModel.to_bettertransformer):

```
model = model.to_bettertransformer()
```

The method [reverse\_bettertransformer()](/docs/transformers/v4.34.0/en/main_classes/model#transformers.PreTrainedModel.reverse_bettertransformer) allows to go back to the original modeling, which should be used before saving the model in order to use the canonical transformers modeling:
