...
## Project Structure
    ...
    │   └── books/
    ├── chroma_transformers/      # Persisted vector database (generated)
...
- The prompt is sent to an OpenAI chat model (ChatOpenAI)
...
Sources: ['data/hf_transformers_docs/xxx.md', ...]
...
- Simple prompt template to enforce grounded answers
- Relevance threshold (0.3) to avoid hallucinations when no good match exists
...
- Fixed chunking strategy (may not be optimal for all documents)
- Uses basic prompt engineering (no advanced prompting or tools)
- No streaming or UI interface
- Depends on OpenAI APIs (cost + latency)
...
- Implement reranking (e.g., cross-encoders)
- Add a web UI (e.g., Streamlit or Gradio)
- Support multiple datasets dynamically
- Improve prompt design (chain-of-thought, citations)
- Add evaluation pipeline for retrieval quality
